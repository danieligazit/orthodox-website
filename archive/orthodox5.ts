...
export default function OrthodoxLabel() {
  ...
  // When rotation is 0 (vertical), we want center at 50vh.
  // When rotation is -90 (horizontal), the "right" edge (now bottom) is at 50vh.
  // We want that edge to be at HEADER_HEIGHT.
  // So we need to shift UP by (50vh - HEADER_HEIGHT).
  const centerOffset = (windowHeight / 2) - HEADER_HEIGHT;
  const bgTranslateY = easeProgress * -centerOffset;

  // 3. Logo Scaling
  // Start huge (e.g., 12vw), End small (header size, e.g., 2rem)
  const startScale = 1; 
  const endScale = 0.2; // Approximate scale down factor
  const currentScale = startScale - (easeProgress * (startScale - endScale));

  // 4. Logo Translation
  // Start: Center of screen (50vh)
  // End: Center of Header (HEADER_HEIGHT / 2)
  ...
  // Start: Separated to visually span across the vertical divide
  // End: 0 tracking to form the word
  // We use padding on the split halves to separate them from the center line.
  const startSeparation = 1; // vw unit - Decreased from 3 for closer initial spacing
  ...
  return (
    <div className="min-h-screen w-full relative bg-[#e8e6df]">
      ...
      <div className="fixed inset-0 flex items-center justify-center overflow-hidden pointer-events-none z-0">
        {/* Container serves as the anchor point (Center of Viewport).
            We translate this container UP so the pivot moves to the header location eventually.
        */}
        <div 
           className="relative"
           style={{
        ...
        >
             {/* THE BLACK PLANE
                Positioned so its RIGHT EDGE is at the exact center of the parent container.
                Dimensions are massive (300vmax) to ensure it covers corners during rotation.
             */}
             <div 
      ...
      {/* 1. mix-blend-difference makes text invert:
             - O (on black) -> White
             - RTHODOX (on white) -> Black
         2. SPLIT LAYOUT:
             - Instead of one centered text block, we use two 50% width blocks.
             - Left Block: Holds "O", Aligned Right (touching center).
             - Right Block: Holds "RTHODOX", Aligned Left (touching center).
         3. SEPARATION:
             - We add dynamic padding (currentSeparation) to the inner edges.
             - This separates them initially and animates them together.
      */}
      <div 
        className="fixed z-50 pointer-events-none mix-blend-difference text-white w-full h-screen flex justify-center"
        style={{
            // We move the container top/down. transform handles centering and scale.
            top: `${currentLogoY}px`,
            left: 0,
