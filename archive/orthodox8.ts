...
export default function OrthodoxLabel() {
  ...
  // 7. Logo Square Opacity (Controls box fade-in)
  // CHANGED: Starts appearing much later (at 80% of the animation) for a snapper finale.
  // Formula: Math.max(0, (easeProgress - startThreshold) * (1 / (1 - startThreshold)))
  const squareOpacity = Math.min(Math.max(0, (easeProgress - 0.8) * 5), 1);

  // 8. O Text Color Calculation
  // Automatically adjusts based on squareOpacity
         ...
         >
            ...
            >
                ...
                <div 
                    ...
                    style={{
                        // Tighter, uniform padding
                        padding: '0.025em',
                        
                        // BACKGROUND: Fades from Transparent -> CSS White
                        backgroundColor: `rgba(232, 230, 223, ${squareOpacity})`,
                        
                        // TEXT COLOR: Fades from CSS White -> CSS Black
                        color: `rgb(${textColorVal}, ${textColorVal}, ${textColorVal})`
                    }}
                >
                    {/* Slight negative margin for optical vertical centering within the tight box */}
